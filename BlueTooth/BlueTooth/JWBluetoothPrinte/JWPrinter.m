...
- (void)setOffsetText:(NSString *)text {
    ...
    valueWidth = valueAttr.size.width;
    // 2.设置偏移量
    if (self.pageSize == 80) {
        [self setOffset:550 - valueWidth];
    } else {
        [self setOffset:364 - valueWidth];
    }
    // 3.设置文字
    [self setText:text];
}
...
- (void)appendLeftText:(NSString *)left middleText:(NSString *)middle rightText:(NSString *)right isTitle:(BOOL)isTitle {
    ...
    if (isTitle) {
        ...
        if (left) {
            if (self.pageSize == 80) {
                int leftLenght = [self convertToInt:left];
                if (leftLenght >= 42) {
                    NSString *oneStr = [left substringToIndex:14];
                    [self setText:oneStr];
                    [self appendNewLine];
                    NSString *twoStr = [left substringWithRange:NSMakeRange(14, 14)];
                    [self setText:twoStr];
                    [self appendNewLine];
                    NSString *threeStr = [left substringFromIndex:28];
                    [self setText:threeStr];
                } else if (leftLenght >= 28 && leftLenght < 42) {
                    NSString *oneStr = [left substringToIndex:14];
                    [self setText:oneStr];
                    [self appendNewLine];
                    NSString *twoStr = [left substringFromIndex:14];
                    [self setText:twoStr];
                } else {
                    [self setText:left];
                }
                ...
                int leftLenght = [self convertToInt:left];
                if (leftLenght >= 36) {
                    NSString *oneStr = [left substringToIndex:9];
                    [self setText:oneStr];
                    [self appendNewLine];
                    NSString *twoStr = [left substringWithRange:NSMakeRange(9, 9)];
                    [self setText:twoStr];
                    [self appendNewLine];
                    NSString *three = [left substringWithRange:NSMakeRange(18, 9)];
                    [self setText:three];
                    [self appendNewLine];
                    NSString *four = [left substringFromIndex:27];
                    [self setText:four];
                } else if (leftLenght >= 27 && leftLenght < 36) {
                    NSString *oneStr = [left substringToIndex:9];
                    [self setText:oneStr];
                    [self appendNewLine];
                    NSString *twoStr = [left substringWithRange:NSMakeRange(9, 9)];
                    [self setText:twoStr];
                    [self appendNewLine];
                    NSString *three = [left substringFromIndex:18];
                    [self setText:three];
                } else if (leftLenght >= 18 && leftLenght < 27) {
                    NSString *oneStr = [left substringToIndex:9];
                    [self setText:oneStr];
                    [self appendNewLine];
                    NSString *twoStr = [left substringFromIndex:9];
                    [self setText:twoStr];
                } else {
                    [self setText:left];
                }
