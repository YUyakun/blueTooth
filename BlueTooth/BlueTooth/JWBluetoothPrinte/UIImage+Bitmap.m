...
- (UIImage *)imageWithscaleMaxWidth:(CGFloat)maxWidth {
    ...
    if (width > maxWidth)
    {
        ...
        NSData *tempData = UIImageJPEGRepresentation(resultImage, 0.1);//压缩图片
        resultImage = [[UIImage alloc]initWithData:tempData];
        return resultImage;
    }
...
+ (UIImage *)qrCodeImageWithInfo:(NSString *)info centerImage:(UIImage *)centerImage  width:(CGFloat)width
{
    ...
    //暂时是图片套图片来解决底部打印不全的问题
    CGRect rect1 = CGRectMake(0, 0, 320, 320);
...
//    UIGraphicsEndImageContext();
//    return resultImage;
    image = [UIImage drawImage:codeImage inImage:image];
